...
const sampleArr = [['A', 'B'], ['A', 'C'], ['B', 'C'], ['C', 'A']]

const ways = []

function itineraries(directions, startPoint){
  if(directions.length==0) return ways

  directions.forEach(el=>{
    if (el[0]==startPoint) {
      ways.push(el[1])
      let dir =  directions.slice(directions.indexOf(el))
      itineraries(dir, el[1])
    }
  })
